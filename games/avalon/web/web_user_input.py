...
class WebUserInput(UserInputBase):
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        """
        # Extract player ID from agent name (e.g., "Player0" -> "0")
        # Use this as the queue key to match with frontend
        import re
        player_id_match = re.search(r'Player(\d+)', agent_name)
        if player_id_match:
            queue_key = player_id_match.group(1)  # Use player ID as queue key
        else:
            queue_key = agent_id  # Fallback to agent_id if name doesn't match pattern
